	...
	AFRAME.registerComponent('forcegraph', {
	  ...
	  init: function () {
	    ...
	    this.el.sceneEl.addEventListener('camera-set-active', function(evt) {
	      ...
	      state.cameraObj = evt.detail.cameraEl.components.camera.camera;
			...
			// Get right laser-controller dom element
			var laserRightEl = document.querySelector('#laser-right');

			// Keep reference to Three laser-controller object
			state.laserRightObj = laserRightEl.components.raycaster.raycaster.ray;
				//.filter(function(child) { return child.type === 'components.raycaster.raycaster' })[0];

			// On laser switch
			this.el.sceneEl.addEventListener('laser-set-active', function(evt) {
				// Switch camera reference
				state.laserRightObj = evt.detail.laserRightObj.components.raycaster.raycaster.ray;
			});
	  ...
	  tick: function(t, td) {
	    ...
	    this.state.forceGraph.tickFrame();
			//console.log(this.state.laserRightObj.origin);
			//console.log(this.state.laserRightObj.direction);
	  }
	});
